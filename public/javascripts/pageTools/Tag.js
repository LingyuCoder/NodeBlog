(function($, window) {
	var __prependTag = function(tag, $div, fnClick) {
		...
		emitter = $(document);
	emitter.bind("tag.drawUserTags", function(event, username, container, fnClick, fnCallback) {
		if (container) {
	...
	}).bind("tag.drawArticleTags", function(event, articleId, container, fnClick, fnCallback) {
		if (container) {
			container.addClass("b-label-loading");
		}
	...
	}).bind("tag.drawAllTags", function(event, container, fnClick, fnCallback) {
		if (container) {
			container.addClass("b-label-loading");
		}
	...
	}).bind("tag.createTag", function(event, name, color, container, fnClick, fnCallback) {
		if (container) {
			container.addClass("b-label-loading");
		}
	...
	}).bind("tag.create", function(event, name, color, fnCallback) {
		$.ajax({
			url: "/nor/tag_create",
			data: {
				name: name,
	...
	}).bind("tag.getAll", function(event, fnCallback) {
		$.ajax({
			url: "/tag_listAll",
			type: "post",
			dataType: "json"
	...
	}).bind("tag.getByUser", function(event, username, fnCallback) {
		$.ajax({
	...
	}).bind("tag.getByArticle", function(event, articleId, fnCallback) {
		$.ajax({
